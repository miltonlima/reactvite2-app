...
let modalidadeLastPageViewAt = 0;
...
async function logModalidadeEvent({ action, statusCode = 200, httpMethod = 'POST', metadata = {} }) {
  try {
    ...
    console.warn('Falha ao registrar log de modalidade:', err);
  }
}
...
function Modalidade() {
  ...
  useEffect(() => {
    const now = Date.now();
    if (now - modalidadeLastPageViewAt >= 1000) {
      modalidadeLastPageViewAt = now;
      logModalidadeEvent({
        action: 'page_view',
        statusCode: 200,
        httpMethod: 'GET',
